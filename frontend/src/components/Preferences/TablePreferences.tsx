...
import { loadColumns } from './configurePreferencesCols';
...
const TableSamplePreferences = ({ filterItems, setFilterItems, filters }) => {
  ...
  useEffect(() => {
    if (preferencesNotify.showNotification) {
      notify(
        preferencesNotify.typeNotification,
        preferencesNotify.textNotification,
      );
    }
  }, [preferencesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/preferences/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/preferences/preferences-view/?id=${id}`);
  };
...
export default TableSamplePreferences;
